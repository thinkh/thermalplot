...
module PVDVisualizations {
  ...
  export class PVDHierarchyGrid extends PVDAHierarchyGrid {
    ...
    constructor($root, private infra:PVDModels.Infrastructure, config: PVDHierarchyConfig, private hierarchiesDown: number = 0) {
      super($root, config);
    ...
    private init(infra:PVDModels.Infrastructure, event: string):void {
      ...
      if(this.layouter === null) {
        this.layouter = this.config.layout.getLayoutById(PVDLayouts.PVDGridLayout.ID);
      }
    ...
    private nextMapping(node: PVDModels.Node) {
    ...
    private attachListener():void {
      ...
      this.config.selection.on('hover'+this.id, (newNode:PVDModels.Node, oldNode: PVDModels.Node) => {
      ...
      // init selection source and target
      {
        ...
        this.config.selection.on('selectall'+this.id+'-2', (newNode:PVDModels.Node, all:PVDModels.Node[], prev: PVDModels.Node[]) => {
