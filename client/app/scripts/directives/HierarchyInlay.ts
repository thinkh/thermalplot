...
module PVDVisualizations {
  ...
  export class PVDHierarchyAInlay implements PVDElementParent, PVDElement {
    ...
    infra:PVDModels.Infrastructure = null;

    protected layout:PVDLayouts.IPVDLayout = null;
    private overlay:PVDHierarchyEdgeOverlay = null;

    protected nodesMap:D3.Map<PVDHierarchyNode> = d3.map();
    nodesPositionMap:D3.Map<PVDLayouts.PVDLayoutDimensions> = d3.map();
    $node: D3.Selection;

    // selected nodes
    selectedSrc:PVDModels.Node = null;
    selectedDst:PVDModels.Node = null; // dst node only used in inlayUp

    // nearest nodes to the selection
    innerSrc:PVDModels.Node = null;
    innerDst:PVDModels.Node = null;
    ...
    createLayout(): PVDLayouts.IPVDLayout {
      var r = new PVDLayouts.PVDNodeLinkLayout(false, false);
      r.drawEdges = true;
      return r;
    }

    updateSelection(all:PVDModels.Node[], position : (node) => { x: number; y : number}) {
    ...
    scaleFactorCb(node:PVDModels.Node):number[] {
      var n = this.nodesMap.get(node.fqIname);
    ...
    createNode(node:PVDModels.Node, index : number = 0):PVDHierarchyNode {
    ...
    protected _updateNodePosition(positions:D3.Map<PVDLayouts.PVDLayoutDimensions>, shift:number[]) {
    ...
    protected _calculateDim(positions:D3.Map<PVDLayouts.PVDLayoutDimensions>):number[] {
    ...
    nodeDimension(node:PVDModels.Node):PVDLayouts.PVDLayoutDimensions {
  ...
  export class PVDHierarchyUpInlay extends PVDHierarchyAInlay {
    ...
    private roots : PVDModels.Node[];
    private srcs : PVDModels.Node[] = [];
    private dsts : PVDModels.Node[] = [];
    ...
    createLayout(): PVDLayouts.IPVDLayout {
      var r = new PVDLayouts.PVDHorizontalNodeLinkLayout();
    ...
    updateSelection(all:PVDModels.Node[], position : (node) => { x: number; y : number}) {
      ...
      var lce = PVDModels.findLeastCommonAncestor(srcs, dsts);
      ...
      dsts.pop();
      var r = PVDModels.graphRoute(a, b);
    ...
    updateRoute(roots: PVDModels.Node[], srcs : PVDModels.Node[] = [], dsts : PVDModels.Node[] = []) {
  ...
  class DownNode {
    children: DownNode[] = [];
    constructor(public node: PVDModels.Node, public parent: DownNode) {
  ...
  export class PVDHierarchyUpInlayStacked extends PVDHierarchyUpInlay {
    ...
    createLayout(): PVDLayouts.IPVDLayout {
      var r = new PVDLayouts.PVDStackLayout(false);
      r.drawEdges = true;
      return r;
    }
  ...
  export class PVDHierarchyDownInlayStacked extends PVDHierarchyDownInlay {
    ...
    createLayout(): PVDLayouts.IPVDLayout {
      var r = new PVDLayouts.PVDStackLayout(true);
      r.drawEdges = true;
      return r;
    }
