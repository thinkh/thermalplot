...
module PVDVisualizations {
  ...
  class ExternalNode implements IVisualNode {
    ...
    constructor(public node:PVDModels.ExternalNode, public parent:IVisualNode, public $root:D3.Selection) {
      this.update();
    }
    ...
    draw() {
      ...
      hub.on('mouseover', function () {
        PVDVisualizations.tooltip().mouseover(that.node.name);
      })
      ...
      */
      hub.call(PVDVisualizations.tooltip(this.node.name));
  ...
  class RectangleNode implements IVisualNode {
    ...
    constructor(public node:PVDModels.Node, public parent:IVisualNode, public $root:D3.Selection, public colors:D3.Scale.OrdinalScale) {
    ...
    draw() {
      ...
      this._d3UpdateSel = this.$root
          .selectAll('.' + PVDModels.converToCSSClass(this.node.fqIname))
          .data([this._rect]);

      this._d3UpdateSel
          .enter()
          .append('rect')
          .attr('class', PVDModels.converToCSSClass(this.node.fqIname))
      ...
      this._d3UpdateSel.call(PVDVisualizations.tooltip(this.node.name));
...
*/
  ...
  class ArcNode implements IVisualNode {
    ...
    draw() {
      ...
      this._d3UpdateSel.enter().append('path')
          .attr('id', function() { return PVDModels.converToCSSClass(that.node.fqIname); })
          .attr('class', function() { return 'infra-node'; })
          .attr('fill', function() { return that.colors(that.siblingsIndex); })
          .each(transformAndPath)
          .call(PVDVisualizations.tooltip(this.node.name))
          //.on('mouseover', function() { PVDVisualizations.tooltip().mouseover(that.node.name); })
          //.on('mousemove', function() { PVDVisualizations.tooltip().mousemove(); })
          //.on('mouseout', function() { PVDVisualizations.tooltip().mouseout(); })
  ...
  export class PVDInfrastructure implements IAnimateable {
    ...
    private _attrList:PVDModels.IAttribute<any>[] = [];
    ...
    constructor(private $svg:D3.Selection, private nodes:PVDModels.Node[], private pvdDataSelection: PVDDataSelection) {
    ...
    private drawBgArcs():void {
      ...
      var externalNode:ExternalNode = new ExternalNode(<PVDModels.ExternalNode>this.nodes.splice(this.nodes.length-1, 1)[0], null, $root);
    ...
    update(dt:number, now:number) : void {
      ...
      var freqList:PVDTimed.TimedParentValue<number,PVDModels.IAttribute<any>>[] = PVDModels.valueList(that._attrList, from, to, step);
      ...
      function getPosition(timedValue:any) {
        ...
        var edge:PVDModels.Edge = <PVDModels.Edge>timedValue.parent.parent;
      ...
      $edges.enter().append('path')
        ...
        .call(PVDVisualizations.tooltip((d: any) => { return d.parent.parent.fqIname + '<br>numConnections = ' + d.v;} ))
        //.on('mouseover', function(d) { PVDVisualizations.tooltip().mouseover(d.parent.parent.fqIname + '<br>numConnections = ' + d.v); })
        //.on('mousemove', function() { PVDVisualizations.tooltip().mousemove(); })
        //.on('mouseout', function() { PVDVisualizations.tooltip().mouseout(); })
        .each(getPosition);
    ...
    calculateEdgeScale(freqList:PVDTimed.TimedParentValue<number,PVDModels.IAttribute<any>>[]) : void {
  ...
  angular.module('pipesVsDamsApp').directive('pvdInfrastructure2', function (pvdAnimator:PVDAnimator, pvdDataSelection:PVDDataSelection, pvdInfrastructureLoader:PVDInfrastructureLoader, $timeout) {
    function initVis($base:D3.Selection, nodes:PVDModels.Node[]) {
