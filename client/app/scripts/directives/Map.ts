/**
 * Created by AK116843 on 26.06.2015.
 */
/**
...
module PVDVisualizations {
  ...
  class PVDMap implements IAnimateable {
    ...
    constructor(private $root, private infra:PVDModels.Infrastructure, private config: PVDHierarchyConfig, private attrs, private dataService) {
    ...
    private attachListener() {
      ...
      this.config.changeBorder.on('crossed'+id, (node:PVDModels.Node, newSegmentRep:SegmentRepresentation) => {
    ...
    prepareData() {
      ...
      that.dataService.when('constantsLoaded').then(() => {
        ...
        function traverseCountries(n: PVDModels.Node, parent) {
          var node = parent;
          if(!n.has()) {
            node = {
              id: n.fqIname,
        ...
        function traverseUSA(n: PVDModels.Node, parent) {
          var node;
          ...
          if(!n.has()) {
            node = {
              ...
              radius: 1,
              state: state,
              latitude: (<PVDModels.ConstantAttribute<any>>n.getAttr('latitude')).getValue(),
              longitude: (<PVDModels.ConstantAttribute<any>>n.getAttr('longitude')).getValue(),
              fillKey: 'bubbles',
              borderWidth: 0,
              color: defaultColor
            };
    ...
    update(dt:number, now:number, layouted:any):void {
      ...
      if(this.useDoiColoring) {
        ...
        that.nodesMap.forEach((fqname, d) => {
          var act = PVDDOI.compute(d.node, s, that.config.selection.doi);
          if (act === undefined) {
            return;
          }
  ...
  angular.module('pipesVsDamsApp').directive('pvdMap', function (pvdInfrastructureLoader:PVDInfrastructureLoader, pvdWindowResize:PVDWindowResize, $timeout, pvdAnimator: PVDAnimator, pvdDataSelection: PVDDataSelection, pvdInfrastructureMapper: PVDInfrastructureMapper, pvdLayoutManager:PVDLayoutManager, pvdTargetHierarchy:PVDTargetHierarchy, pvdChangeBorder:PVDChangeBorder, pvdDataService:PVDDataService) {
    return  {
