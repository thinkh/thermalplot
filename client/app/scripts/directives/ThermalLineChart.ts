  ...
  export class PVDThermalLineChart implements PVDElementParent, IAnimateable {
    ...
    private initLayout(infra:PVDModels.Infrastructure):void {
      ...
      that.line = d3.svg.line()
        .x((d:PVDDOI.DeltaDOI) => this.time(d.ts))
        .y((d:PVDDOI.DeltaDOI) => this.doi(d.doi))
        .interpolate('linear');
    ...
    private computeTrajectoriesActivities(node: PVDModels.Node, s: PVDSelection, f: PVDDOI.DOIFormula):PVDDOI.DeltaDOI[] {
      return PVDDOI.computeTrajectory(node, s, f);
    }
    ...
    layout(dt:number, now: number) : any {
      ...
      var data:D3.Map<PVDDOI.DeltaDOI[]> = d3.map(),
          activity;
    ...
    update(dt:number, now:number, data : D3.Map<PVDDOI.DeltaDOI[]>) : void {
      var that = this;
