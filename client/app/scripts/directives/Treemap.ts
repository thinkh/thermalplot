...
module PVDVisualizations {
  ...
  class PVDTreemap implements IAnimateable {
    ...
    private prepareData() {
      ...
      function traverse(n: PVDModels.Node, parent) {
        var node = {
          'id': n.fqIname,
          'name': n.title,
          'node': n,
    ...
    update(dt:number, now:number, layouted:any):void {
      ...
      if(this.useDoiColoring) {
        ...
        that.nodesMap.forEach((fqname, node) => {
          var act = PVDDOI.compute(node.node, s, that.config.selection.doi);
          if (act === undefined) {
            return;
          }
