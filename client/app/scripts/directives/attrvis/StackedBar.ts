...
module PVDVisualizations {
  ...
  export class PVDStackedBar implements PVDElement, PVDInnerElement {
    ...
    constructor($parent:D3.Selection, public attr:PVDModels.IAttribute<number>, private type:string, private config:PVDHierarchyConfig, private parent: PVDElementParent, public defConfig:any) {
      this.$node = $parent.append('div').attr('class', name + ' stackedbar');

      this.calculateValuesByLevel();

      var id = 'dragEnd.stackedbar_'+PVDVisualizations.nextID();
    ...
    private calculateValuesByLevel() {
      ...
      function mapDown(n : PVDModels.Node, level: number) {
        var children = that.config.mapper.mapToUnchecked(n, target[level], []).map((ni) => {
          //downNodes.push(ni);

          var attr:PVDModels.IAttribute<number> = <PVDModels.IAttribute<number>>(ni.getAttr(that.attr.name));
          if(attr !== undefined) {
    ...
    private draw(width:number, height:number):void {
      this.$node.call(PVDVisualizations.tooltip(this.toTooltip()));
    ...
    private toTooltip() {
      ...
      this.valuesByLevel.forEach((d,i) => {
        ...
        r.push(''+this.format(d)+' '+(<PVDModels.NumberAttribute>this.attr).unit);
        r.push("</td></tr>\n");
      });
      r.push("</tbody></table>");
      return r.join('');
