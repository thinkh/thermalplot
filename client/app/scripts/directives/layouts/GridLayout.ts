...
module PVDLayouts {
  ...
  export class PVDGridLayout extends PVDALayout implements IPVDLayout {
    ...
    private positionNodes(bundle:PVDLayoutBundle, gridWidth:number, gridHeight:number, baseWidth:number, baseHeight:number):void {
      ...
      var node:PVDModels.Node = null,
          queue:PVDModels.Node[] = [],
          leaves:PVDModels.Node[] = [];
    ...
    private positionInlayBelowGrid(inlay:PVDVisualizations.PVDHierarchyAInlay, nodeDimMap:D3.Map<PVDLayoutDimensions>, gridWidth:number, gridHeight:number, baseWidth:number, baseHeight:number) {
      if (inlay === null) {
        return;
    ...
    private positionLeaves(nodeDimMap:D3.Map<PVDLayoutDimensions>, leaves:PVDModels.Node[], baseWidth:number, baseHeight:number):void {
      var that = this,
        inlayUp = this.inlayUp,
        recalculate = false,
        scale = [],
    ...
    private offsetX(node:PVDModels.Node):number {
      var sum = this.summedChildrenMap.get(node.fqIname);
      if(sum === undefined) { sum = 0; }
      return this.offsetIntermediate[0] * sum;
    }

    private getSiblings(node:PVDModels.Node):PVDModels.Node[] {
      var that = this,
        result = [];
    ...
    private hasIntermediateSiblings(node:PVDModels.Node):boolean {
      var that = this,
        result = false;
    ...
    private groupMasterNodes(bundle: PVDLayoutBundle) {
      var that = this;
      function bundleThem(node: PVDModels.Node, masters: PVDModels.Node[]) {
      ...
      function t(node: PVDModels.Node) {
