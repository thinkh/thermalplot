...
module PVDLayouts {
  ...
  export class PVDNodeLinkLayout extends PVDANodeLinkLayout implements IPVDLayout {
    ...
    private drawTheEdges(graph: any, edges: PVDVisualizations.PVDLayoutEdge[], voffset: number) {
      var t = (node:any) => {
        var p = node.parent;
        if (node.masters.length > 1) {
          node.masters.forEach((m) => edges.push(PVDVisualizations.edgeBetween(m.dim, new PVDLayoutDimensions(node.x, m.y2+voffset, node.width, 0),m.node.fqIname + '-master')));
        }
        if (!p) {
          return;
        }
        ...
        case 0:
          edge = PVDVisualizations.edgeBetween(p.dim, node.dim, node.node.fqIname + '-' + p.node.fqIname);
          break;
        case 1:
          edge = PVDVisualizations.edgeBetween(p.masters[0].dim, node.dim, node.node.fqIname + '-' + p.node.fqIname);
          break;
        default:
          edge = PVDVisualizations.edgeBetween(new PVDLayoutDimensions(p.x, node.y-voffset, p.width, 0), node.dim, node.node.fqIname + '-' + p.node.fqIname);
          break;
        }
        if (edge) {
          edges.push(edge);
