...
module PVDDOI {
  ...
  export function compute(node: PVDModels.Node, s: PVDSelection, f:  DOIFormula) {
  ...
  export function computeWindow(node:PVDModels.Node, s:PVDSelection, f:DOIFormula):DOIWindow {
    if (node.getAttr('doi') === undefined) {
  ...
  export function computeTrajectory(node:PVDModels.Node, s:PVDSelection, f:DOIFormula):DeltaDOI[] {
    if (node.getAttr('doi') === undefined) {
  ...
  export function computeValidDataPoints(node:PVDModels.Node, s:PVDSelection, f:DOIFormula):number {
    if (node.getAttr('doi') === undefined) {
      return 0;
    }
    var attr:PVDModels.DOIAttribute = <PVDModels.DOIAttribute>(<any>node.getAttr('doi'));
  ...
  export class DOIValues {
    ...
    constructor(attrs:PVDModels.IAttribute<number>[], private from:number, private to:number, private step:IStepper) {
...
module PVDModels {
  ...
  export class DOIAttribute extends Attribute<number> {
    constructor(name: string, alias: string, parent: IPathElem, private formula : PVDDOI.DOIFormula = new PVDDOI.DOIFormula(), public invert:boolean = false) {
    ...
    setFormula(f : PVDDOI.DOIFormula) {
    ...
    private first(ts: number) {
      ...
      var values = new PVDDOI.DOIValues(this.attrs, from, ts, this.formula.stepper);
      var toValue = (i) => {
    ...
    private intermediate(doi_tm1 : PVDDOI.DOIValue, ts: number) {
      if (doi_tm1.ts >= ts) { //bigger due to fuzzy days
        return doi_tm1;
      }
      var values = new PVDDOI.DOIValues(this.attrs, this.formula.stepper.step(doi_tm1.ts), ts, this.formula.stepper);
      var toValue = (i) => {
        var values_i = values.getAt(i);
        return this.formula.evalX(values_i);
      };
     ...
     * @returns {number}
     */
    private forecast(doi_tm1 : PVDDOI.DOIValue, ts: number) {
      //volatile
    ...
    private compute(ts: number, returnNullAsDefault : boolean, noForecasting : boolean): number {
      ...
      if (!attrs.every((a) => this.fuzzyHas(a,ts, prev_v === null))) {
        ...
        if (prev_v) { //we have something to continue
          prev_doi = this.intermediate(new PVDDOI.DOIValue(prev_v.v, prev_v.duration, prev_v.ts), common);
        } else {
        ...
        if (prev_v) { //fi
          return this.intermediate(new PVDDOI.DOIValue(prev_v.v, prev_v.duration,prev_v.ts), ts).s;
        }
        //first one
    ...
    normalize(value: number): number {
      return PVDDOI.norm(value, this.formula.range);
    }
    ...
    rawValues(from:number, to:number):PVDTimed.TimedValue<number>[] {
      ...
      for(; from <= to; from = this.formula.stepper.step(from)) {
        ...
        r.push(new PVDTimed.TimedValue(from, v));
      }
      return r;
    }
