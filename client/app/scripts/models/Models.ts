...
module PVDModels {
  import TimedIndexedValue = PVDTimed.TimedIndexedValue;
  /**
  ...
  export interface IAttribute<T> extends IPathElem {
    ...
    rawValues(from:number, to:number):PVDTimed.TimedValue<T>[];
     ...
     * @param step
     */
    valueList(from:number, to:number, step:IStepper, r: PVDTimed.TimedParentValue<T,IAttribute<T>>[]):PVDTimed.TimedParentValue<T,IAttribute<T>>[];
    /**
     * see TimedList#floor
     * @param from
     * @param to
     * @param step
     */
    floor(ts:number):PVDTimed.TimedIndexedValue<T>;
     ...
     * @param step
     */
    ceiling(ts:number):PVDTimed.TimedIndexedValue<T>;
  ...
  export class Attribute<T> implements IAttribute<T> {
    ...
    protected data = new PVDTimed.TimedList<T>();
    protected invalidValue = null;
    public binner : PVDTimed.IBinner<T> = null;
    ...
    valueList(from:number, to:number, step:IStepper, r = new Array<PVDTimed.TimedParentValue<T,IAttribute<T>>>()):PVDTimed.TimedParentValue<T,IAttribute<T>>[] {
  ...
  export class NumberCalcAttribute extends NumberAttribute {
    ...
    valueList(from:number, to:number, step:IStepper, r = new Array<PVDTimed.TimedParentValue<number,IAttribute<number>>>()):PVDTimed.TimedParentValue<number,IAttribute<number>>[] {
    ...
    rawValues(from:number, to:number):PVDTimed.TimedValue<number>[] {
    ...
    floor(ts:number):PVDTimed.TimedIndexedValue<number> {
      var a = this.attrs;
      if (a.length == 0) {
        return null;
      }
    ...
    ceiling(ts:number):PVDTimed.TimedIndexedValue<number> {
      var a = this.attrs;
      if (a.length == 0) {
        return null;
      }
  ...
  export class ACompositeAttribute<T> implements IAttribute<T> {
    ...
    valueList(from:number, to:number, step:IStepper, r = new Array<PVDTimed.TimedParentValue<T,IAttribute<T>>>()):PVDTimed.TimedParentValue<T,IAttribute<T>>[] {
      ...
      var tmp = new Array<PVDTimed.TimedParentValue<T,IAttribute<T>>>(),
        a = this._attrs();
      ...
      nest.forEach((group) => {
        ...
        r.push(new PVDTimed.TimedParentValue(this,group.values[0].ts,val,step.refStepWidth));
      });
      return r;
    }
  ...
  export class LinearTransformedAttribute<T> extends Attribute<T> {
    ...
    private fobj(v): any {
      ...
      if (v instanceof PVDTimed.TimedParentValue) {
        return new PVDTimed.TimedParentValue(v.parent, v.ts, this.f(v.v), v.duration);
      }
      if (v instanceof PVDTimed.TimedIndexedValue) {
        return new PVDTimed.TimedIndexedValue(v.ts, this.f(v.v), v.duration, v.index);
      }
      return new PVDTimed.TimedValue(v.ts, this.f(v.v), v.duration);
    }
  ...
  export class ConstantAttribute<T> extends Attribute<T> {
    ...
    rawValues(from:number, to:number):PVDTimed.TimedValue<T>[] {
      if (this.isSet) {
        return [new PVDTimed.TimedValue(from, this.value)];
      }
      return [];
    }
    ...
    valueList(from:number, to:number, step:IStepper, r = new Array<PVDTimed.TimedParentValue<T,IAttribute<T>>>()):PVDTimed.TimedParentValue<T,IAttribute<T>>[] {
      for(var i= from; i < to; i = step.step(i)) {
        r.push(new PVDTimed.TimedParentValue(this, i, this.value));
      }
      return r;
    }
    ...
    floor(ts:number):PVDTimed.TimedIndexedValue<T> {
      ...
      return new PVDTimed.TimedIndexedValue(ts, this.value,0, 0);
    }
   ...
   * @returns {any[]}
   */
  export function valueList<T>(attrs: IAttribute<T>[], from : number, to: number, step: IStepper) : PVDTimed.TimedParentValue<T,IAttribute<T>>[] {
    var r = new Array<PVDTimed.TimedParentValue<T,IAttribute<T>>>();
   ...
   * @returns {aPVDTimed.TimedParentValue<T,IAttribute<T>>}
   */
  export function intermediateRoutes<T>(list : PVDTimed.TimedParentValue<T,IAttribute<T>>[]) : PVDTimed.TimedParentValue<T,IAttribute<T>>[] {
    var m = d3.map();
    //create the chain entries
    function createChain(chain: PVDModels.Node[], v : PVDTimed.TimedParentValue<T,IAttribute<T>>) {
      for(var i = 0; i < chain.length-1; ++i) {
        ...
        if (m.has(key)) {
          ...
          m.set(key, new PVDTimed.TimedParentValue<T,IAttribute<T>>(attr, v.ts, v.v, v.duration));
        }
      }
    }
    //for each entry
    list.forEach((v) => {
      if (!(v.parent.parent instanceof PVDModels.Edge)) {
        return;
      }
      var edge : PVDModels.Edge = <PVDModels.Edge><any>v.parent.parent,
   ...
   * @param attr
   * @param parent
   * @returns {PVDModels.AttributeRedirect}
   */
   ...
   * @param name
   * @param alias
   * @returns {PVDModels.CompositeFunctionAttribute<number>}
   */
  export function composeF(attrs: () => IAttribute<number>[], reduce = '+', parent: IPathElem = null, name : string = null, alias : string = null) {
    var r = new PVDModels.CompositeFunctionAttribute<number>(parent, name, alias);
    r.attrs = attrs;
