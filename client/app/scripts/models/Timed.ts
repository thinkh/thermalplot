...
module PVDTimed {
  ...
  export class ImmutableTimedList<T> {
     ...
     * @param start
     * @param end
     * @returns {PVDTimed.ImmutableTimedList<T>}
     */
    ...
    valueList<P>(parent: P, from: number, to: number, step: IStepper, binner: IBinner<T>, feedForward: boolean, filter: (T) => boolean, invalidValue:T = null, r = new Array<PVDTimed.TimedParentValue<T,P>>()): TimedParentValue<T,P>[] {
    ...
    frequencyList<P>(parent: P, from:number, to:number, step:IStepper, artificialStart: boolean = true, r = new Array<PVDTimed.TimedParentValue<number,P>>()): TimedParentValue<number,P>[] {
      if (this.isEmpty) { //no data at all
        return r;
      }
