  ...
  private addListener() {
    ...
    this.pvdDataService.on('internalMessage', (msg) => {
      ...
      switch(msg.subtype) {
        ...
        {
          var doi = msg.doi;
          var components = doi.components.map((c) => new PVDDOI.DOIComponent(c.attr, c.weight, c.range, c.invert));
          that.pvdDataSelection.doi = new PVDDOI.DOIFormula(components, doi.alpha, doi.beta, doi.nsteps, doi.default_, doi.range, doi.step, doi.fuzzyDays, doi.deltaMethod, doi.loadingPercentage);
          break;
        }
        case 'indexPoint':
        {
...
angular.module('pipesVsDamsApp').service('pvdBroadcastService', PVDBroadcastService);
