...
import { PVDHierarchyUpInlay } from './HierarchyInlay';
import { IAnimateable } from '../services/Animator';
import { PVDStreamGraph, PVDStackedBars } from './attrvis/StreamGraph';
import { PVDHeatmap, PVDCompositeAttributeHeatmap, PVDGradientHeatmap } from './attrvis/Heatmap';
import { PVDTimeAxis } from './attrvis/TimeAxis';
import { PVDHorizontalBar } from './attrvis/HorizontalBar';
import { PVDSingleStream } from './attrvis/SingleStream';
import { PVDStockStream } from './attrvis/StockStream';
import { PVDMeanMinMax, PVDCategoryStack } from './attrvis/MeanMinMax';
...
export class PVDHierarchyNode implements PVDElement, PVDElementParent {
  ...
  hasNodeChildren() {
    ...
    return !this.collapsed && (this.node.has() || this.node instanceof ExternalNode);
  }
  ...
  private createStackedBar(defId: string, defAttr: string, defConfig: any) {
    var attr: IAttribute<number> = <IAttribute<number>>(this.node.getAttr(defAttr));
    if (attr === undefined) {
  ...
  private createHeatMap(defId: string, defAttr: string, defConfig: any) {
    ...
    if (!a && this.collapsed) {
      ...
      if (attrs.length > 0) {
        a = compose(attrs, 'mean', this.node, defAttr);
      }
    }
    if (!a) {
      return;
    ...
    switch (type) {
      ...
      case 'categoryhist':
        ...
        break;
      case 'doistream':
        var doistream = new PVDDOIStreamGraph(this.$node, <DOIAttribute>(<any>a), normalizer, this.config, this, defConfig);
  ...
  private createConnectionHeatMap(defId: string, defAttr: string, defConfig: any) {
    ...
    var a = compose(attrs, '+', this.node, defAttr);
    var r = new PVDHeatmap(this.$node, a, this.config.streamNormalizer, this.config, this, defConfig);
    this._currentChildren.push(r);
    this._allChildrenMap.set(defId, r);
  }
  ...
  private createStreamGraph(defId: string, defAttr: string, defConfig: any) {
    ...
    var type: any = defConfig.interpolate ? PVDStreamGraph : PVDStackedBars;
...
class TimeHighlight implements IAnimateable {
  ...
  update(dt: number, now: number, layouted: any) {
    ...
    if (ts) {
      tooltip().update(this.createTooltip(ts.ts));
    }
  }
