...
export class Infrastructure {
  ...
  forEachAttr(func: (attr: IAttribute<any>) => void) {
    function traverseImpl(node: Node) {
...
export function findLeastCommonAncestor(sparents: Node[], dparents: Node[]) {
  var i: number = sparents.length - 1, j: number = dparents.length - 1, p: Node = null;
