...
import { DOIFormula } from '../models/DOI';

/**
 * the current selection everything in milliseconds [ms]
 */
export class PVDSelection {
...
export class PVDDataSelection {
  ...
  private _selections: Node[] = [];
  private _infra: Infrastructure = null;
  private _hover: Node = null;
  ...
  addBulkSelection(selection: Node[], replaceExisting = true) {
    var s = this._selections;
    if (replaceExisting) {
      this._selections = selection;
    } else {
  ...
  removeFromSelection(selection: Node) {
  ...
  getSelectionAsUnchecked(id: string, _else: Node[] = []) {
    var s = this.selection;
    if (s) {
   ...
   * @returns {*}
   */
  getSelectionsAsUnchecked(id: string, _else: Node[] = []) {
   ...
   * @param selection
   */
  set infra(infra: Infrastructure) {
...
export default angular.module('services.pvdDataSelection', [
  Animator,
