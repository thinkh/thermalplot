...
import { PVDInfrastructureMapper } from '../services/InfrastructureMapper';
import { PVDDataSelection } from '../services/DataSelection';
...
export class PVDHierarchyConfig {
  ...
  showInNode(hasChildren: boolean, node: Node) {
  ...
  activityOf(node: Node) {
    //FIMXE compute store and update ActivityManager
    return 1;
  }

  clone(): PVDHierarchyConfig {
    // copy config to overwrite config values in inlayUp
    var c: any = new PVDHierarchyConfig(this.animator, this.selection, this.layout, this.mapper, this.targetHierarchy, this.changeBorder, this.windowResize);
    for (var i in this) {
      if (i !== 'streamNormalizer' && i !== 'nodeWidth' && i !== 'startTime') {
        c[i] = this[i];
      }
    }
    return <PVDHierarchyConfig>c;
  }
